...
class KdenParse:
    ...
    def getMainTractor(self):
        tractorList = []
        tractors = self.xmldoc.getElementsByTagName("tractor")
        for t in tractors:
            tid = t.attributes["id"].value
            if tid == "maintractor":
                return t
        raise "maintractor was not found"
    
    def getGivenPlaylist(self,pid):
        playlists = self.xmldoc.getElementsByTagName("playlist")
        for p in playlists:
            pid2 = p.getAttribute("id")
            if pid2 == pid:
                return p
        raise Exception("this playlist: "+pid+" wasnt found")
    
    def getGivenProducer(self,pid):
        producers = self.xmldoc.getElementsByTagName("producer")
        for p in producers:
            pid2 = p.getAttribute("id")
            if pid2 == pid:
                return p
        raise Exception("this producer: "+pid+" wasnt found") 
